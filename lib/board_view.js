...
class BoardView {
  ...
  updateCurrentScore() {
    ...
    if (parseInt(this.scoreElement.innerHTML) !== currentScore) {
      ...
      if (currentScore > parseInt(this.highScoreElement.innerHTML)) {
        this.highScoreElement.innerHTML = this._padScore(currentScore, 10);
      }
    }
  }

  getHighScore() {
    const highScore = sessionStorage.getItem('high_score') || 0;
    this.highScoreElement.innerHTML = this._padScore(highScore, 10);
  }

  setHighScore() {
    sessionStorage.setItem('high_score', this.board.currentScore);
  }
  ...
  updateLives() {
    ...
    if (livesDiff > 0) {
      ...
      displayLives.addChild(displayLives.lastElementChild);
    }
  }
